...
AQL
  ...
  used on all {term}`openEHR` systems to get data from stored EHRs.


Cohort
  A combination and parametrized selection of {term}`Criteria` to restrict data output to a subset
  of EHRs, e.g. "Vaccinated persons over the age of 18" or "All female persons over the age of 60".
